...
impl JobRepo {
    ...
    pub async fn create(&self, uri: &Uri) -> Result<Job> {
        ...
        sqlx::query_as!(
            Job,
            r#"
                INSERT INTO job (uri) VALUES (?1) RETURNING id as "id: JobId", uri as "uri: JobUri";
            "#,
            uri_str
        )
        .fetch_one(&self.pool)
    ...
    #[allow(unused)]
    async fn get_one(&self, id: JobId) -> Result<Job> {
        sqlx::query_as!(
            Job,
            r#"
                SELECT 
...
mod test {
    ...
    async fn test_repo(pool: SqlitePool) {
        ...
        let job = repo
            .get_one(job.id)
            .await
            .expect("failed to retrieved job for test");
